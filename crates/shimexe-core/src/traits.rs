...
/// Type alias for pre-execute hook functions
pub type PreExecuteHook = Box<dyn Fn(&[String]) -> Result<()> + Send + Sync>;

/// Type alias for post-execute hook functions
pub type PostExecuteHook = Box<dyn Fn(i32) -> Result<()> + Send + Sync>;
...
impl ShimRunnerBuilder {
    ...
    pub fn build(self, shim_name: &str, shim_dir: &Path) -> Result<CustomizableShimRunner> {
        let config_file_pattern = self
            .config_file_pattern
            .unwrap_or_else(|| "{name}.shim.toml".to_string());

        let config_file = shim_dir.join(config_file_pattern.replace("{name}", shim_name));
...
impl ShimRunnerTrait for CustomizableShimRunner {
    async fn execute(&self, additional_args: &[String]) -> Result<i32> {
        ...
        {
            self.check_and_update(update_provider.as_ref(), version_checker.as_ref())
                .await?;
        }
...
impl CustomizableShimRunner {
    ...
    async fn check_and_update(
        &self,
        update_provider: &dyn UpdateProvider,
        version_checker: &dyn VersionChecker,
    ) -> Result<()> {
        let current_version = "1.0.0"; // TODO: Get from config or executable

        if let Some(latest_version) = update_provider
            .check_update_available(current_version)
            .await?
        {
            if version_checker.is_newer_version(&latest_version, current_version)? {
                let executable_path = self.config.get_executable_path()?;
                update_provider
                    .install_update(&latest_version, &executable_path)
                    .await?;
            }
        }
