    ...
    pub fn execute(&self, shim_dir: Option<PathBuf>) -> Result<()> {
        ...
        let auto_update = match self.provider.as_str() {
            "github" => {
                ...
                let asset_pattern = self.asset_pattern.as_ref()
                    .ok_or_else(|| anyhow::anyhow!("--asset-pattern is required for GitHub provider"))?;
                ...
                AutoUpdate {
                    ...
                    download_url: format!("https://github.com/{}/releases/download/{{version}}/{}", repo, asset_pattern),
            ...
            "https" => {
                return Err(anyhow::anyhow!("HTTPS provider configuration not yet implemented"));
            }
            "custom" => {
                return Err(anyhow::anyhow!("Custom provider configuration not yet implemented"));
            }
            _ => {
...
impl ConfigureAutoUpdateCommand {
    pub fn execute(&self, shim_dir: Option<PathBuf>) -> Result<()> {
        ...
        let mut auto_update = config.auto_update
        ...
        if let Some(prerelease) = self.prerelease {
            match &mut auto_update.provider {
                UpdateProvider::Github { include_prerelease, .. } => {
            ...
            match &mut auto_update.version_check {
                VersionCheck::GithubLatest { include_prerelease, .. } => {
                    *include_prerelease = prerelease;
                }
                _ => {}
            }
...
impl StatusAutoUpdateCommand {
    ...
    fn show_single_status(&self, manager: &ShimManager, name: &str) -> Result<()> {
        ...
        if let Some(ref auto_update) = config.auto_update {
            ...
            println!("   Provider: {}", self.format_provider(&auto_update.provider));
            println!("   Check interval: {} hours", auto_update.check_interval_hours);
